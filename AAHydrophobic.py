...
def gather0_charges_and_lambdas(sequence):
    ...
    lambdas = {f'a{i}': first5_vals[i]*0 for i in range(num_modes)}
...
def gather_charges_and_lambdas(sequence):
    ...
    first5_vecs = eigvecs[:, :num_modes]
    aa_index = {aa:i for i,aa in enumerate(aa_order)}
    charges = {f'a{i}': np.array([first5_vecs[aa_index[res], i] for res in sequence])
            for i in range(num_modes)}
    lambdas = {f'a{i}': first5_vals[i] for i in range(num_modes)}
    return charges, lambdas

def get_es_charges(sequence):
    aa_charges = {
    'A': 0.0,  'C': 0.0,  'D': -1.0, 'E': -1.0,
    'F': 0.0,  'G': 0.0,  'H': 0.1,  'I': 0.0,
    'K': 1.0,  'L': 0.0,  'M': 0.0,  'N': 0.0,
    'P': 0.0,  'Q': 0.0,  'R': 1.0,  'S': 0.0,
    'T': 0.0,  'V': 0.0,  'W': 0.0,  'Y': 0.0, 'pb': 0.0, 'Nsc': 0.0, 'Csc':0.0
    }
    ...
    residue_charges = {}
    for res in sequence.upper():
        if res not in residue_charges:
            residue_charges[res] = aa_charges.get(res, 0.0)
    if 'pb' not in residue_charges:
        residue_charges['pb'] = aa_charges.get('pb', 0.0)
    residue_charges['Nsc'] = aa_charges.get('Nsc', 0.0)
    residue_charges['Csc'] = aa_charges.get('Csc', 0.0)
    
    # Solvent charges
    solvent_charges_dict = {key: val for key, val in solvent_charges.items()}
    
    es_charges = {**residue_charges, **solvent_charges_dict}
    return es_charges